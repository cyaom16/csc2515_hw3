...
imds = imageDatastore('D:\CSC411\Data\train\train');
imds.Labels=categorical(string(csvread('D:\CSC411\Data\train.csv',1,1,[1 1 7000 1])));

testSet = imageDatastore('D:\CSC411\Data\val');
...
testSet.ReadFcn = @(filename)readAndPreprocessImage(filename);
%%
...
[trainingSet, validateSet] = splitEachLabel(imds, 0.8, 'randomize');
...
% |validateSet|. The validate features can then be passed to the classifier to
% measure the accuracy of the trained classifier.

% Extract validate features using the CNN
validateFeatures = activations(convnet, validateSet, featureLayer, 'MiniBatchSize',32);
testFeatures = activations(convnet, testSet, featureLayer, 'MiniBatchSize',32);

% Pass CNN image features to trained classifier
predictedLabels = predict(classifier, validateFeatures);
testLabels = predict(classifier, testFeatures);
% Get the known labels
validateLabels = validateSet.Labels;

% Tabulate the results using a confusion matrix.
confMat = confusionmat(validateLabels, predictedLabels);
...
labelOut(1:970,2)= (int64(grp2idx(testLabels)));
labelOut(971:2970,2)= 0;
labelOut(:,1)= linspace(1,2970,2970);
csvwrite('testLabels.csv',labelOut);
